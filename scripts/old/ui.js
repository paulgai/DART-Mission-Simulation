...
export function initUI(state, callbacks) {
  ...
  // Zoom (scale)
  //bindPair("#scaleRange", "#scaleNum", "scale");

  bindPair("#speedRange", "#speedNum", "speed");
  // Checkboxes
  /*$("#stopOnCrash").addEventListener(
    "change",
    (e) => (state.stopOnCrash = e.target.checked)
  );
  $("#limitPath").addEventListener(
    "change",
    (e) => (state.limitPath = e.target.checked)
  );*/
...
export function updateOrbitUI(state) {
  ...
  if (orbitTypeEl) {
    orbitTypeEl.textContent = type;
    orbitTypeEl.className = "pill " + pillClass;
  }
  // τιμές
  setText("uCirc", Number.isFinite(uc) ? uc.toFixed(3) + " m/s" : "—");
  setText("uEsc", Number.isFinite(ue) ? ue.toFixed(3) + " m/s" : "—");
  setText("energy", Number.isFinite(E) ? E.toFixed(3) + " J/kg" : "—");
  setText("ecc", Number.isFinite(e) ? e.toFixed(3) : "—");
