...
export function initUI(state, callbacks) {
  ...
  if (showImagesEl) {
    showImagesEl.addEventListener("change", (e) => {
      ...
      // Κάθε φορά που αλλάζει το mode εικόνες/σημεία:
      // - σταματά η προσομοίωση
      // - επιστροφή στις αρχικές συνθήκες
      // - ΠΛΗΡΗΣ επαναυπολογισμός τροχιάς (με τη νέα crashR)
      state.running = false;
  ...
  $("#showImages").addEventListener(
    "change",
    (e) => (state.showBodyImages = e.target.checked)
  );
  ...
  $("#showStars").addEventListener(
    "change",
    (e) => (state.showStars = e.target.checked)
  );
